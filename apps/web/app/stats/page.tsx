...
const HOUR_BUCKETS = [
  { from: 0, label: '00' }, { from: 3, label: '03' },
  { from: 6, label: '06' }, { from: 9, label: '09' },
  { from: 12, label: '12' }, { from: 15, label: '15' },
  { from: 18, label: '18' }, { from: 21, label: '21' },
]

function moodFromValence(v: number) {
...
export default async function StatsPage() {
  ...
  try {
    const [u, ap, hm, gn] = await Promise.all([
      query<UserMeta>(`SELECT display_name, avatar_url FROM raw.users WHERE id = $1`, [session.userId]),
  ...
  // Bucketize heatmap into 8 x 7 grid
  ...
  return (
    <>
      ...
      <main className="min-h-screen px-6 md:px-8 lg:px-12 pt-8 md:pt-12 pb-32 md:pb-16 max-w-md md:max-w-6xl mx-auto">
        ...
        <div className="font-mono text-[11px] text-text-mute uppercase tracking-widest mb-1">stats</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          ...
          <section className="bg-surface border border-border rounded-2xl p-5 md:p-6">
            <h2 className="text-sm font-bold uppercase tracking-wider mb-4">¿Cuándo escuchas?</h2>
            {hasHeatmap ? (
              <>
                <div className="grid grid-cols-[28px_repeat(7,1fr)] gap-1 mb-1">
                  <div />
                  {DAY_LABELS.map((d) => (
                    <div key={d} className="text-center font-mono text-[10px] text-text-mute">{d}</div>
                  ))}
                </div>
                <div className="grid grid-cols-[28px_repeat(7,1fr)] gap-1">
                  {HOUR_BUCKETS.map((b, bi) => (
                    <Fragment key={bi}>
                      <div className="font-mono text-[10px] text-text-mute flex items-center justify-end pr-1">{b.label}</div>
                      {Array.from({ length: 7 }).map((_, day) => {
                        const v = grid[bi][day]
                        const intensity = v / maxCell
                        const bg = v === 0 ? 'var(--color-border)' : `rgba(255, 45, 146, ${0.25 + intensity * 0.75})`
                        return (
                          <div
                            key={`c${bi}-${day}`}
              ...
              <p className="text-text-dim text-sm text-center py-6">
                Aún no hay datos suficientes para el heatmap.
              </p>
            )}
          </section>
          ...
          {/* GÉNEROS */}
          <section className="bg-surface border border-border rounded-2xl p-5 md:p-6">
            <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Tus géneros principales</h2>
            {hasGenres ? (
              <div className="flex flex-wrap gap-2">
                {topGenres.map((g, i) => {
                  ...
                  const color = i === 0
                    ...
                    : 'bg-bg-soft border border-border text-text-dim'
                  return (
              ...
              <div className="text-center">
                ...
                <p className="text-text-mute text-[11px] mt-3">
                  Si los géneros vienen vacíos, es porque Spotify los reserva para Extended Quota Mode.
                </p>
              </div>
            )}
          </section>
          ...
          {hasAudio && (
            <section className="lg:col-span-2 bg-surface border border-border rounded-2xl p-5 md:p-6">
              <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Huella sonora · audio profile</h2>
          ...
          {!hasAudio && (
            <section className="lg:col-span-2 bg-surface border border-dashed border-amber/30 rounded-2xl p-5 md:p-6 text-center">
...
function Headline({ profile }: { profile: AudioProfileRow }) {
  ...
  return (
    <div className="rounded-2xl p-5 mb-5 relative overflow-hidden"
      style={{ background: 'linear-gradient(135deg, rgba(255, 45, 146, 0.18), rgba(0, 212, 255, 0.10))' }}>
...
function FeatureGrid({ profile }: { profile: AudioProfileRow }) {
  const features: Array<{ key: keyof AudioProfileRow; label: string; color: string }> = [
    ...
    { key: 'avg_instrumentalness', label: 'instr.', color: 'var(--color-purple)' },
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2.5">
      {features.map((f) => {
        ...
        return (
          <div key={f.key} className="bg-bg-soft border border-border rounded-xl p-3.5">
            <div className="flex items-baseline justify-between mb-2">
              <span className="font-mono text-[10px] text-text-mute uppercase tracking-widest">{f.label}</span>
